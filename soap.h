...
namespace NS_selftabel {
	...
	// ************************************************************************ //
	__interface INTERFACE_UUID("{51BBADE7-41EC-D5B1-174D-593F7258BD48}")
		selftabelPortType : public IInvokable {
	public:
		virtual UnicodeString CheckIn(const __int64 Apt, const UnicodeString BC,
			const __int64 isnight) = 0;
		virtual UnicodeString CheckOut(const __int64 Apt,
			const UnicodeString BC, const __int64 isnight) = 0;
		virtual UnicodeString Test() = 0;
		virtual __int64 GetAllowedAction(const __int64 Apt,
			const UnicodeString BC) = 0;
		virtual bool AptIsSelfTabeling(const __int64 AptCode) = 0;
	};
	typedef DelphiInterface<selftabelPortType>_di_selftabelPortType;

	_di_selftabelPortType GetselftabelPortType(bool useWSDL = false,
		System::String addr = System::String(),
		Soaphttpclient::THTTPRIO* HTTPRIO = 0);
